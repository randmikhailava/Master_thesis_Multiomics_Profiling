...
run_gene_test_satterthwaite <- function(gene, df) {
  tryCatch({
    ...
    # 1. Basic checks: NAs and Variance
    if (sum(!is.na(y_values)) < 3) {
      return(list(gene = gene, status = "error", message = "Not enough values", result = NA))
    }
    ...
    # 3. SINGULARITY CHECK
    # This detects if the variance of (1|Sample) is effectively zero
    ...
    # 5. Return Results with Status Warning
    return(list(
      gene = gene,
      # Label as 'singular' so you can filter or inspect these later
      status = if(is_singular) "singular_warning" else "success",
      message = if(is_singular) "Singular fit: random effect variance is zero" else NA,
      result = c(
